 ...
 *
 * Streams real tracker modules (.xm / .s3m / .mod) through libopenmpt (WASM)
 * via the vendored `chiptune3` AudioWorklet player in /js/vendor/.
 *
 ...
 * action (see NOTES-chiptune.md for the app.js wiring).
 *
 ...
 * instance with its own (null) AudioContext. Parent bumps the pack to ?v=5.
 */
...
class Chiptune {
  constructor() {
    ...
    this._tracks  = [];      // track list (manifest.json .tracks or injected)
    this._catalog = null;    // catalogue injected by the server (see setCatalog)

    this._idx     = -1;      // index of the track last handed to the worklet
    this._playing = false;
    this._vol     = 0.85;
  ...
  async list() {
    ...
    this._manifestPromise = fetch(MANIFEST_URL, { credentials: 'same-origin' })
      ...
      .then((j) => {
        const t = Array.isArray(j && j.tracks) ? j.tracks : [];
  ...
  /**
   * Start the station at `index`. `onStart(trackMeta)` fires when a track's
   * audio actually begins (once per track, including auto-advances). `onEnd()`
   * fires only when the station can no longer continue (every track failed) -
   * the parent uses it to bring the generative bed back.
   */
  async play(index, opts = {}) {
    try {
      ...
      this._failStreak = 0;
      await this._loadIndex(this._wrap(index));
    } catch (err) {
      console.warn('[chiptune] play failed:', (err && err.message) || err);
      this._safeEnd(opts.onEnd);
    }
  }

  async _loadIndex(idx) {
    ...
    const meta = this._tracks[idx];
    if (!meta || !meta.file) return this._advanceAfterFailure(token);

    let buf;
    try {
      const res = await fetch(TRACK_BASE + meta.file, { credentials: 'same-origin' });
  ...
  _onMeta() {
    ...
    this._playing = true;
    this._failStreak = 0;
    this._emitStart(this._trackMeta(this._idx));
  }
  ...
  _advanceAfterFailure(token) {
    if (token !== this._loadToken) return;
    this._failStreak++;
    if (this._failStreak >= (this._tracks.length || 1)) {
      this._playing = false;
      this._failStreak = 0;
      this._safeEnd(this._cbs && this._cbs.onEnd);
      return;
    }
    return this._loadIndex(this._wrap(this._idx + 1));
  }
  ...
  _onTrackEnded() {
    ...
    Promise.resolve()
      .then(() => this._loadIndex(this._wrap(this._idx + 1)))
      .catch((err) => console.warn('[chiptune] auto-advance failed:', (err && err.message) || err))
      .finally(() => { this._advancing = false; });
  }

  next() {
    if (!this._ready || !this._tracks.length || this._advancing) return;
    this._advancing = true;
    Promise.resolve()
      .then(() => this._loadIndex(this._wrap(this._idx + 1)))
      .catch(() => {})
  ...
  prev() {
    ...
    this._advancing = true;
    Promise.resolve()
      .then(() => this._loadIndex(this._wrap((this._idx < 0 ? 0 : this._idx) - 1)))
      .catch(() => {})
      .finally(() => { this._advancing = false; });
  }
  ...
  _emitStart(meta) {
    try { this._cbs && this._cbs.onStart && this._cbs.onStart(meta); }
